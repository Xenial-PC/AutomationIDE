...
namespace AutomationIDELibrary.ReservedKeyWords
{
    public class Functions
    {
        ...
        public Task RunCSScriptAsync(string script)
        {
            RunCSScriptFuncAsync(script);
            return Task.CompletedTask;
        }
        #endregion
        ...
        private Task BaseFunctionRunCSScriptAsync(string script)
        {
            ...
            var code = $@"
                ...
                //css_reference {Directory.GetCurrentDirectory()}\Dlls\WebDriver.Support.dll;
                //css_reference {Directory.GetCurrentDirectory()}\Dlls\Selenium.WebDriver.Extensions.dll;
                //css_reference {Directory.GetCurrentDirectory()}\Dlls\System.IO.dll;
            
                using static AutomationIDELibrary.Compiler.Compiler;
                using static AutomationIDELibrary.ReservedKeyWords.Functions;
                using OpenQA.Selenium;
                using OpenQA.Selenium.Support.UI;
                using SeleniumExtras.WaitHelpers;
                using System.IO;
        ...
        private Task BaseFunctionRunCSScriptFromPathAsync(string path)
        {
            CSScript.GlobalSettings.AutoClass_DecorateAsCS6 = true;
            CSScript.AssemblyResolvingEnabled = true;
            CSScript.ShareHostRefAssemblies = true;
        ...
        private Task RunCSScriptFuncAsync(string keyword)
        {
            if (keyword.StartsWith("CSScript", StringComparison.Ordinal))
                BaseFunctionRunCSScriptAsync(keyword.Remove(0, 8));
            else if (keyword.StartsWith("P:CSScript", StringComparison.Ordinal))
                BaseFunctionRunCSScriptFromPathAsync(keyword.Remove(0, 10));
            return Task.CompletedTask;
        }
        #endregion
    }
